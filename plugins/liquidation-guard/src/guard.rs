...
/// Infinity LST mint (see the job report's Deviations section).
...
fn run_check(call: &ParsedCall, transport: &mut dyn Transport) -> Result<String, String> {
    ...
    let obligation = select_obligation(obligations, call.obligation.as_deref())?;
...
fn run_deposit(call: &ParsedCall, transport: &mut dyn Transport) -> Result<String, String> {
    ...
    // from `obligation_reserves`, see the job report), this pipeline's own
...
mod tests {
    ...
    fn pinned_lst_mints_match_fixture() {
        ...
        // INF has no row in this fixture — its mint is cross-checked
        // against Sanctum's published Infinity LST mint instead (see
        // docs/jobs/liquidation-guard/fx-lst-sol-level-01.md, Deviations).
        assert!(
