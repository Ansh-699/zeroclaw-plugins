...
//! and liquidate remain structurally impossible: no encoder for them
//! exists anywhere in this module, verified by the grep-enforced forbidden
//! surface in `docs/checks/liquidation-guard/rescue.md`.
...
/// single-sample assumption, see the job report's Deviations section.
const TOKEN_PROGRAM_ID: &str = "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA";

/// Compute-unit ceiling for the priority-fee `SetComputeUnitLimit`
/// instruction, pinned from the golden fixture's real measured cost:
/// `tests/fixtures/repay_tx.json` `meta.computeUnitsConsumed` = 261_070.
/// 261_070 * 1.5 (safety margin for account/price-account count variance
/// across obligations) = 391_605, rounded up to the clean 400_000.
/// `pub(crate)` so `report::render_rescue` can name the same number it
/// built into the tx.
pub(crate) const RESCUE_CU_LIMIT: u32 = 400_000;
...
/// `ReserveCollateral.mint_pubkey` — empirically located (see job report)
...
/// field-observed mainnet deposit instruction sequence (job report,
/// v11-deposit-encoder: signature
...
pub fn build_deposit_tx(
    ...
    //    17 accounts (job report account-mapping table), no remaining
    //    accounts.
