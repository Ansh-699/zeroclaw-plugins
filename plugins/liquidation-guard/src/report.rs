...
/// edge; internal math stays fractions everywhere else).
...
fn forecast_line(symbol: &str, cmp: &str, threshold: f64, now: f64, sol_level: bool) -> String {
    let mut line = format!(
        "Liquidated if {symbol} {cmp} ${threshold:.2} (now ${now:.2}, {}%)",
...
fn remedy_line(m: &PositionMeta, r: &Remedy) -> String {
    ...
    let mut line = format!(
        "{verb} {:.1} {symbol} \u{2192} LTV {}%, buffer {}% (needs {:.1} {symbol} in wallet)",
        r.ui_amount,
        pct(r.resulting_ltv),
...
pub fn render_rescue(m: &PositionMeta, r: &RescueText, snapshot: &str) -> String {
    let mut out = format!(
        "{custody}\n\n\
         ...
         Repay {repay_ui:.1} {debt} ({amount_native} native units) — capped by {capped_by}.\n",
    ...
    out.push_str(&format!(
        "Requires {repay_ui:.1} {debt} in wallet.\n\
         tx (base64): {tx}\n\n\
         snapshot: {snapshot}",
        repay_ui = r.repay_ui,
        debt = r.debt_symbol,
...
pub fn render_deposit(m: &PositionMeta, d: &DepositText, snapshot: &str) -> String {
    let mut out = format!(
        "{custody}\n\n\
         Obligation: {obligation} ({market})\n\
         Deposit {deposit_ui:.1} {collateral} ({amount_native} native units) — capped by {capped_by}.\n",
        custody = CUSTODY_SENTENCE,
        obligation = m.obligation,
        market = m.market,
        deposit_ui = d.deposit_ui,
    ...
    out.push_str(&format!(
        "Requires {deposit_ui:.1} {collateral} in wallet.\n\
         tx (base64): {tx}\n\n\
         snapshot: {snapshot}",
        deposit_ui = d.deposit_ui,
        collateral = d.collateral_symbol,
