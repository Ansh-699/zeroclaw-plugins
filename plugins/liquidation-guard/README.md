...
| `rpc_url`      | `https://api.mainnet-beta.solana.com`             | https-only; used only for `getLatestBlockhash` / balance reads.           |
...
every account's pubkey/signer/writable flags — from
`tests/fixtures/repay_tx.json`/`deposit_tx.json` (the captured txs) and
`tests/fixtures/reserve_accounts.json` (the reserve account blobs).
...
3. **Repay + deposit only; withdraw/borrow/liquidate remain structurally
   ...
   `repay_and_withdraw_and_redeem` exists anywhere in `src/` — grepping the
   whole tree for any of those four names returns no match (amended per the
...
→ `rescue` against the explicit `obligation` arg produces the **exact same**
output byte-for-byte as the clean fixture — the decoy is filtered out by the
`obligation` match and none of its text ever reaches the amount/account
computation; no request in either run left the closed endpoint set
(`injected_payload_strings_never_alter_amounts`). The same decoy fixture
proves the identical invariant for `deposit`
...
→ `check` still succeeds and renders that string **verbatim as inert display
data** in the normal remedy-line position (e.g. `Repay 8568.4 <that string> →
...`); the report still ends with a `snapshot:` line as normal; no request
left the closed endpoint set (`injected_symbol_text_renders_as_inert_data`).
...
Repay 8568.4 USDG → LTV 59.9%, buffer 25.0% (needs 8568.4 USDG in wallet)
Deposit 0.2 cbBTC → LTV 59.9%, buffer 25.0% (needs 0.2 cbBTC in wallet)
...
Repay 8568.4 USDG (8568438980 native units) — capped by computed.
Requires 8568.4 USDG in wallet.
...
paths from `tests/live_evidence.rs` (`#[ignore]`d, run explicitly, no network
access in the normal `cargo test` gate), plus a `simulateTransaction` call
made by `curl` outside the plugin — the plugin itself still has no
`sendTransaction`/`simulateTransaction` call anywhere in `src/`.

| artifact                                | value |
| ----------------------------------------- | ------- |
| Release wasm artifact                     | `cargo build --locked --target wasm32-wasip2 --release` → 554,467 bytes (v0.2.0; +19,768 over the pre-`overflow-checks` build for the integer trap paths, +627 for the `ui_to_native` scaling gate — the cost of not wrapping and not saturating) |
...
- **Token-2022 collateral mints.** `build_deposit_tx`'s
  ...
  program (single empirical sample, a SOL reserve) — see the job report's
  Deviations section.
