...
/// reserve for the obligation — see the job report) — so
...
/// v11-deposit-encoder job report) genuinely writes to that account:
...
fn fee_on_prepends_compute_budget_ixs() {
    ...
    // RESCUE_CU_LIMIT is not exported; re-derive the same clean constant
    // from the fixture's computeUnitsConsumed here rather than importing a
    // private const, matching the derivation comment in src/rescue.rs.
    const RESCUE_CU_LIMIT: u32 = 400_000;
